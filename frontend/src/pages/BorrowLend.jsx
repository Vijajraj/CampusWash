...
export default function BorrowLend() {
  ...
  const { listings, totalPages, loading, error, refetch } = useListings(filters, page);
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-6 md:px-12 py-10">
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-surface border border-border rounded-xl shadow-sm">
            <p className="text-sm text-text-muted">No listings found matching your search.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {listings.map((listing) => (
              <LendCard key={listing.id} listing={listing} onRefresh={refetch} />
            ))}
          </div>
        )}
